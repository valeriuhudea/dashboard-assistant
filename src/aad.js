    /*
      if (ok_ses_end_resp !== undefined && aad_ses_end_resp != undefined) {
        const ok_resp_status = JSON.stringify(ok_ses_end_resp.status)
        const aad_resp_status = JSON.stringify(aad_ses_end_resp.status)
        if (ok_resp_status == 204 && aad_resp_status == 200) {
          var bothOktaAAD = true
          res.render('success', { successMessage: "Session termination successful in Okta and AAD!" })
        } else {
          res.json({ message: "Something went wrong, bad request or insuficient permissions.", status: ok_resp_status })
        }
      } else {
